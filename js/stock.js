...
function get_max_price(stock_data, start_index, end_index) {
    ...
    for(var i = start_index; i < stock_data.length && i <= end_index; i++ ) {
        if (i==start_index) {
            max_price = parseFloat(stock_data[i].item_hignest_price);
        }
        else {
            max_price = Math.max(max_price, parseFloat(stock_data[i].item_hignest_price));
        }
    }
    return max_price;
}
...
function get_min_price(stock_data, start_index, end_index) {
    ...
    for(var i = start_index; i < stock_data.length && i <= end_index ; i++ ) {
        if (i==start_index) {
            min_price = parseFloat(stock_data[i].item_hignest_price);
        }
        else {
